...
//        Ngasih power extra
    ...
    public boolean straightTo(Pose2d currentPos, Pose2d targetPosition, Pose2d heading) {
        ...
        double curX = odo.getPosX(INCH);
        double curY = odo.getPosY(INCH);

        double x = targetPosition.position.x - curX;
        double y = targetPosition.position.y - curY;
        // Tank drive 2 motor
        ...
        double drivePower = straightPID.calculateInches(error, 0);
...
//        klo gk pake signum ntr clip hasilnya malah 0.6 bukan - 0.6 jadi yang harusnya mundur malah ma
...
//        tpi tetep bisa ke degree 0 biar gk salah jalur
        double currentYaw = odo.getHeading(AngleUnit.DEGREES);
        double headingCorrection = turnPID.angleWrapDegree(currentYaw - heading.heading.toDouble());
